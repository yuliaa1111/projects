...
class Evaluator:
    ...
    def __init__(
        ...
        # rankic_refit_roll support (optional)
        future_concat: Optional[Dict[str, Any]] = None,

        # existing factor evaluation system bridge (optional; server-only)
        ...
        # future concat backtest (optional; keep old behavior by default)
        self.future_concat = dict(future_concat or {})
        self.future_concat_enabled = bool(self.future_concat.get("enabled", False))
        self.future_part = str(self.future_concat.get("future_part", "future"))
        self.future_subdir = str(self.future_concat.get("out_subdir", "future"))
        self.future_preds_filename = str(self.future_concat.get("preds_filename", "future_preds_all.parquet"))
        self.future_validate_no_overlap = bool(self.future_concat.get("validate_no_overlap", True))
        self.future_save_fig = bool(self.future_concat.get("save_fig", self.save_fig))

        # factor eval bridge (optional)
        self.factor_eval = dict(factor_eval or {})
        self.factor_eval_enabled = bool(self.factor_eval.get("enabled", False))
    ...
    def run(self) -> EvalResult:
        ...
        try:
            # future_concat needs future part to be loaded
            if self.future_concat_enabled and self.future_part not in self.parts:
                self.parts = list(self.parts) + [self.future_part]
            ...
            # --------------------------------------------------
            # Optional: future prediction concat backtest (rankic_refit_roll)
            # --------------------------------------------------
            if self.future_concat_enabled:
                self._run_future_concat(pred_df)

            # --------------------------------------------------
            # Optional: existing factor evaluation system bridge (server-only)
            # --------------------------------------------------
            if self.factor_eval_enabled:
                self._run_factor_eval()
    ...
    # -------------------------
    # Future concat (rankic_refit_roll)
    # -------------------------
    def _run_future_concat(self, pred_df: pd.DataFrame) -> None:
        """
        For rankic_refit_roll:
        - Strictly separate test evaluation vs future prediction.
        - Stitch all future predictions (part='future') into one continuous out-of-sample series.
        - Save future_preds_all.parquet + metrics_future.csv (+ optional plots) under out_dir/future/.
        """
        if "part" not in pred_df.columns:
            logger.warning("future_concat skipped | pred_df missing 'part' column")
            return

        df_test = pred_df[pred_df["part"] == "test"].copy()
        df_future = pred_df[pred_df["part"] == self.future_part].copy()

        # Save separated metrics for test evaluation
        if not df_test.empty:
            test_metrics = self._compute_metrics(df_test)
            test_metrics_path = os.path.join(self.out_dir, "metrics_test.csv")
            test_metrics.to_csv(test_metrics_path, index=False, encoding="utf-8-sig")
            logger.info("test metrics saved | path=%s | shape=%s", test_metrics_path, test_metrics.shape)
        else:
            logger.warning("future_concat: no test rows found (part='test'); metrics_test.csv skipped")

        if df_future.empty:
            logger.warning("future_concat skipped | no future rows found (part='%s')", self.future_part)
            return

        # Validate overlap if possible
        if self.future_validate_no_overlap:
            needed = {self.date_col, self.stockid_col}
            if not needed.issubset(set(df_future.columns)):
                raise KeyError(
                    f"future_concat.validate_no_overlap needs columns {sorted(list(needed))}, got: {list(df_future.columns)}"
                )
            dup = int(df_future.duplicated([self.date_col, self.stockid_col]).sum())
            if dup > 0:
                raise ValueError(f"future_concat detected overlapping future preds: duplicate (date,stockid) rows={dup}")

        df_future = df_future.sort_values([self.date_col, self.stockid_col]).reset_index(drop=True)

        future_out_dir = os.path.join(self.out_dir, self.future_subdir)
        os.makedirs(future_out_dir, exist_ok=True)

        # Save stitched future preds
        future_preds_path = os.path.join(future_out_dir, self.future_preds_filename)
        df_future.to_parquet(future_preds_path, index=False)
        logger.info("future preds stitched saved | path=%s | rows=%d", future_preds_path, int(len(df_future)))

        # Compute metrics on stitched future preds (treated as a single part)
        df_future2 = df_future.copy()
        df_future2["part"] = "future_all"
        future_metrics = self._compute_metrics(df_future2)
        future_metrics_path = os.path.join(future_out_dir, "metrics_future.csv")
        future_metrics.to_csv(future_metrics_path, index=False, encoding="utf-8-sig")
        logger.info("future metrics saved | path=%s | shape=%s", future_metrics_path, future_metrics.shape)

        # Optional future plots (save into out_dir/future)
        if self.future_save_fig:
            orig_out_dir = self.out_dir
            orig_stitch_filename = self.stitch_filename
            try:
                self.out_dir = future_out_dir
                self.stitch_filename = "future_regression_all_plots_long.png"

                raw_paths = self._make_plots(df_future2)
                if self.task == "regression" and self.stitch_regression:
                    long_path = os.path.join(future_out_dir, self.stitch_filename)
                    stitched = self._stitch_regression_plots(raw_paths, long_path)
                    if stitched and (not self.stitch_keep_individual):
                        for p in raw_paths:
                            try:
                                ...
                                pass
                    logger.info("future plots done | stitched=%s", str(stitched))
                else:
                    logger.info("future plots done | n=%d", len(raw_paths))
            finally:
                self.out_dir = orig_out_dir
                self.stitch_filename = orig_stitch_filename

    # -------------------------
    # Existing factor evaluation system bridge (server-only)
    # -------------------------
    def _run_factor_eval(self) -> None:
        """
        Run the existing factor evaluation system on properly-dispatched prediction source.

        Notes:
        - This runs only when evaluate.params.factor_eval.enabled=true.
        - Intended to run on server (clickhouse settings path required by run_pred_factor_eval.py).
        - For rankic_refit_roll, default dispatch uses stitched future preds (NOT test evaluation preds).
        """
        from .factor_eval_bridge import run_factor_eval_from_preds

        cfg = dict(self.factor_eval or {})
        strategy = cfg.get("strategy")
        if not strategy:
            raise KeyError("factor_eval.enabled=true requires evaluate.params.factor_eval.strategy")

        input_source = str(cfg.get("input_source", "auto"))

        # Where to read preds
        pred_glob = str(cfg.get("pred_glob", self.pred_glob))
        part_name = str(cfg.get("part_name", "test"))
        future_parquet_path = cfg.get("future_parquet_path")

        # What value column to treat as factor signal
        value_col = str(cfg.get("value_col", "y_pred"))

        # Output layout (subdirs keeps backward compatibility; separate_dirs matches tasks.md example naming better)
        excess_output_layout = str(cfg.get("excess_output_layout", "subdirs"))
        meta_filename = str(cfg.get("meta_filename", "factor_eval_meta.json"))

        # complete_factor_analysis params: must be fully provided by config (no implicit defaults)
        analysis_params = cfg.get("analysis_params")
        if not isinstance(analysis_params, dict) or len(analysis_params) == 0:
            raise KeyError("factor_eval.enabled=true requires evaluate.params.factor_eval.analysis_params (dict)")

        # Output naming: require out_dir; allow simple placeholders
        out_dir_tpl = cfg.get("out_dir")
        if not out_dir_tpl:
            raise KeyError("factor_eval.enabled=true requires evaluate.params.factor_eval.out_dir")
        ...
        source_tag = str(
            cfg.get(
                ...
                (
                    "future_preds"
                    if str(strategy).lower() == "rankic_refit_roll" and str(input_source).lower() in ("auto", "future_stitched")
                    else f"{part_name}_preds"
                ),
            )
        )
        excess_mode = str(cfg.get("excess_mode", "both"))

        out_dir = (
            str(out_dir_tpl)
            .replace("{eval_dir}", str(self.out_dir))
            .replace("{strategy}", str(strategy))
            .replace("{input_source}", str(input_source))
            .replace("{source_tag}", str(source_tag))
            .replace("{excess_mode}", str(excess_mode))
        )

        logger.info(
            "factor_eval start | strategy=%s | input_source=%s | source_tag=%s | out_dir=%s",
            str(strategy),
            str(input_source),
            str(source_tag),
            str(out_dir),
        ...
        run_factor_eval_from_preds(
            ...
            eval_dir=str(self.out_dir),
            date_col=str(self.date_col),
            stockid_col=str(self.stockid_col),
        ...
        logger.info("factor_eval done | out_dir=%s", str(out_dir))
